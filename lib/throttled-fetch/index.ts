...
export class RequestQueue<T = any> {
  ...
  /**
   * The number of milliseconds each interval lasts.
   */
  ...
  constructor({
     ...
     * terminating:
     *
     *   - Mutate `addRequestToQueue`'s params before letting it continue.
     ...
     * processing of other requests or the period of intervals, making it ideal
     * for more complex (e.g. isolated, per-endpoint) throttling requirements.
     *
     ...
     *   - Throw an error causing the `addRequestToQueue` method to reject.
     *
     * Delaying a request using `fetchErrorInspector` will have no effect on the
     * processing of other requests or the period of intervals, making it ideal
     * to retry failed fetch requests.
     *
     ...
     * the queue immediately without worrying about managing the scheduling
     * runtime.
     *
   ...
   * If this function returns `undefined` or a promise that resolves to
   * `undefined`, an internal `fetch()` will be made using the request params
   * passed to (and potentially mutated by) this function. The fetch result will
   * be passed to `responseInspector`. Otherwise, the resolved defined value of
   * this function will be passed to `responseInspector` directly (no internal
   * `fetch()` happens).
   *
   * If this function throws, the corresponding `addRequestToQueue` call will
   * reject and `responseInspector` will not be called.
   ...
   *
   * Delaying a request using `fetchErrorInspector` will have no effect on the
   * processing of other requests or the period of intervals, making it ideal to
   * retry failed fetch requests.
   *
   * The resolved value of this function will always be passed to
   * `responseInspector` directly (no additional internal `fetch()` happens)
   * unless an error is thrown, in which case the `addRequestToQueue` return
      ...
      for (; count < this.maxRequestsPerInterval; ++count) {
        ...
        void (async () => {
          try {
            ...
            const requestInspectorResult =
              await this.requestInspector(inspectorParams);
  ...
  delayRequestProcessingByMs(delay: number) {
    ...
    debug(
      `set delayRequestProcessingByMs: ${this.#delayRequestProcessingByMs} => ${delay}`
    );
